...
class IronWeb3Provider extends BaseProvider {
  constructor(config) {
    super(config);
  ...
  setAddress(address) {
    ...
    try {
      for (var i = 0; i < window.frames.length; i++) {
        ...
        if (frame.ethereum && frame.ethereum.isIron) {
  ...
  request(payload) {
    ...
    var that = this;
    if (!(this instanceof IronWeb3Provider)) {
    ...
    switch (payload.method) {
      ...
      default:
        throw new ProviderRpcError(
          4200,
          `Iron does not support calling ${payload.method} synchronously without a callback. Please provide a callback parameter to call ${payload.method} asynchronously.`
        );
    }
    return response;
  }
...
module.exports = IronWeb3Provider;
